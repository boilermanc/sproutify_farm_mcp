...
// MCP tool handler
async function getMCPData(toolName, params = {}) {
...
app.post('/sage', async (req, res) => {
    try {
        ...
        const getMCPDataWithFarm = async (toolName, params = {}) => {
            return getMCPData(toolName, { ...params, farmId });
        };
        const response = await sage.processMessage(message, { farmName: farmName || 'Your Farm' }, getMCPDataWithFarm);
        res.json({ response });
    }
    catch (error) {
